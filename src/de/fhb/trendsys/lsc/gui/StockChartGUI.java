...
public class StockChartGUI extends Application {
	...
	/**
	 * erzeugt die Tabs für das Fenster mit Größe und Bezeichnungen
	 * 
	 * @param root
	 */
	...
	/**
	 * initialiseren der GUI-Komponeten mit Postionsangaben
	 * 
	 * @param stage
	 */
	...
	/**
	 * erzeugen des mauellen Update-Button
	 * 
	 * @return Button
	 */
	...
	/**
	 * aktualisert und baut den Ticker neu
	 */
	 ...
	 * @return TilePane
	 */
	private TilePane createStockNewsPane() {
	...
	/**
	 * aktualisert und baut die StockNewsPane(NewsListe rechts) neu
	 * 
	 * @param name
	 *            - Aktienbezeichnung
	 */
	private void refreshStockNewsPane(String name) {
		...
		if (currentChart != null) {
			...
			for (final NewsVO feed : currentChart.getNewsFeeds()) {
				...
				Hyperlink actualLink = HyperlinkBuilder
						.create()
						.textFill(Color.BLACK)
						.font(Font.font("Verdana", FontWeight.BOLD, 10))
						.text(this.model.millisToHHMM(feed.getTime().getTime())
								+ " Uhr  " + feed.getTitle())
						.tooltip(new Tooltip(feed.getUrl())).build();

				actualLink.setOnAction(new EventHandler<ActionEvent>() {
					@Override
					public void handle(ActionEvent e) {
						tabPane.getSelectionModel().select(webTab);
						webContainer.getWebEngine().load(feed.getUrl());
					}
				});
	...
	/**
	 * erzeugt den Ticker im unteren Fensterrand
	 * 
	 * @return Group
	 */
	private Group createNewsTicker(Stage stage) {
	...
	/**
	 * Methode zur Erzeugung der News-Einträge des Tickers
	 * 
	 * @param hyperlinks
	 *            - Liste der Nodes des Tickers
	 */
	private void addNewsToTicker(List<Node> hyperlinks) {
		...
		for (final NewsVO feed : this.model.getTickerNews()) {
			...
			Hyperlink actualLink = HyperlinkBuilder.create()
					...
					.font(Font.font("Verdana", FontWeight.BOLD, 10))
					.text(feed.getTitle()).translateY(3)
					.tooltip(new Tooltip(feed.getUrl())).build();
			...
			hyperlinks.add(TextBuilder.create().text(" + ").translateY(3)
	...
	/**
	 * Methode zur Erzeugung der Text-Nodes für Prozent-Einträge im Tickers
	 * 
	 * @param hyperlinks
	 *            - Liste der Nodes des Tickers
	 */
	private void addStockPercentagesToTickerNodes(List<Node> hyperlinks) {
		for (ChartVO currentChart : this.model.getChartList()) {
			...
			Hyperlink chartLink = HyperlinkBuilder.create()
					.textFill(Color.WHITE)
					.font(Font.font("Verdana", FontWeight.BOLD, 10))
					.text(seriesName).translateY(3)
					.tooltip(new Tooltip("Anzeigen von " + seriesName)).build();
			...
			if (stockDayDiff >= 0.01d) {
				hyperlinks.add(TextBuilder.create()
						.text(String.format("+%.2f", stockDayDiff) + "%")
						.font(Font.font("Verdana", FontWeight.BOLD, 12))
						.translateY(3).fill(Color.LIGHTGREEN).build());
			} else if (stockDayDiff < 0d) {
				hyperlinks.add(TextBuilder.create()
						.text(String.format("%.2f", stockDayDiff) + "%")
						.font(Font.font("Verdana", FontWeight.BOLD, 12))
						.translateY(3).fill(Color.RED).build());
			} else {
				hyperlinks.add(TextBuilder.create()
						.text(String.format("+/-%.2f", stockDayDiff) + "%")
						.font(Font.font("Verdana", FontWeight.BOLD, 12))
						.translateY(3).fill(Color.YELLOW).build());
			}
	...
	/**
	 * erzeugt die Choice-Box mit ChangeListener und verknüpften Model
	 * (chartNamesList)
	 * 
